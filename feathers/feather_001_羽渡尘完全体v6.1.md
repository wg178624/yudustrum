---
title: "羽渡尘完全体 v6.1 — 全量集成"
tags: [羽渡尘, 架构, 总览, v6.1, 全量]
type: doc
created: 2026-05-28
---
...
| 信念 | 说明 |
|------|------|
| 法无禁止皆可为 | 灰色地带可以接受，但不出格。不出格的标准是不影响槟城计划 |
| 二八定律 | 80%收益来自20%客户。不追求低级市场 |
| 不做国人生意 | 售后心累单价低，倾向国际市场 |
| 对机器绝对信任 | 机器没有自由意志，不会背叛。对人有防备 |
...
| **v5.9.0** | **5/28** | **五层审计(Defender风格) + Nginx反代 + MySQL + 冗余备份** |
| **v6.0.0** | **5/29** | **羽渡尘进化: 记忆→学习系统. OCR读教材(中/英). 技能融合7in1. 浏览器+CF上网搜东西. EIE私教+C合体. ROI引擎. 控制论审计.** |
| **v6.1.0** | **6/3** | **API通信优化: append-only加载保前缀稳定. 自动压缩防缓存膨胀. 过滤reasoning tokens省token. 借鉴Reasonix对话管理策略.** |
...
1. **五层审计**
   - 第一层：文件完整性检查（关键脚本哈希）
   - 第二层：敏感数据扫描（`grep` 身份证/API Key）
   - 第三层：依赖漏洞检测（Python 包安全公告）
   - 第四层：权限审计（`700` 目录、`.env` 未提交）
   - 第五层：行为审计（`audit.py` 记录异常操作）

2. **网络隔离**
   - Nginx 监听 8081，对外暴露；`server.py` 监听 127.0.0.1:8080，仅本地代理
   - 未配 HTTPS（本地环境，暂不需要）

3. **数据备份**
   - 实时：每次 `bake_dashboard.py` 后自动备份 `status.json` 到 `D:\408\`
   - 定时：每周 Git 全量提交 + 推送 GitHub
   - 冗余：D盘 + 备用目录 + GitHub（异地在美）

4. **密钥管理**
   - API Key、VPS 密码等已从 `USER.md` 移入 `~/.hermes/secrets/.env`，且 `.gitignore` 排除
   - 仪表盘未暴露公网，无需额外认证
